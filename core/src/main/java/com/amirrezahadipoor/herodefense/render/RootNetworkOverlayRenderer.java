...
public final class RootNetworkOverlayRenderer implements AutoCloseable {
    ...
    public void draw(
        ...
        // Tree trunk background gradient
        ...
        // Draw tree via sapling renderer if available (idle sway)
        if (treeRenderer != null) {
            // We reuse sapling draw at center for visual anchor
            // Actual full-screen tree is background; nodes are overlays
        }

        // Draw connections (lines) for prerequisites
        batch.end();

        Gdx.gl.glEnable(GL20.GL_BLEND);
        shapes.setProjectionMatrix(projection);
        shapes.begin(ShapeRenderer.ShapeType.Filled);
        // Draw lines between required nodes
        ...
        for (RootNodeDefinition def : RootNetworkCatalog.all()) {
            ...
            if (purchased) {
                shapes.setColor(0.85f, 0.78f, 0.25f, 1f); // gold lit
            } else if (canBuy) {
                shapes.setColor(0.45f, 0.65f, 0.45f, 0.9f); // green affordable
            } else {
                shapes.setColor(0.25f, 0.25f, 0.25f, 0.7f); // locked
            }
        ...
        for (RootNodeDefinition def : RootNetworkCatalog.all()) {
            ...
            if (purchased) {
                text.drawCentered(batch, \"\\u2713\", def.x(), def.y() + 28f, 0.8f, OverlayText.GOLD, 1f);
            }
        }
        ...
        // Selected node details (show first affordable or hovered? For now show feedback)
