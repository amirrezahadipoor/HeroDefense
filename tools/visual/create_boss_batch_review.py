...
def audit_batch(baseline: Path, candidate: Path) -> dict:
    ...
    if candidate_manifest.get("generatedBatch") != "bosses":
        raise ValueError("Candidate must be generated by the exact bosses batch")
    if candidate_manifest.get("maxAtlasPageSize") != 2048:
        raise ValueError("Candidate max atlas page size must remain 2048")
...
def validate_metadata(
    ...
    if entry.get("sheets") != [expected_sheet]:
        raise ValueError(f"{key} must remain on one exact 2048x1024 atlas page")
