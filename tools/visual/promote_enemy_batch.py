...
def validate_review_evidence(
    ...
    for key, (revision, rig_profile, animation_profile) in EXPECTED.items():
        ...
        if committed_sheet.is_file():
            committed_hash = sha256_file(committed_sheet)
            permitted = {record["baselineSheetSha256"], record["candidateSheetSha256"]}
            if committed_hash not in permitted:
                raise ValueError(f"Committed {key} no longer matches reviewed baseline or candidate")
