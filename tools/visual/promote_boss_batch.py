#!/usr/bin/env python3
"""Promote only the exact hash-audited premium-v2 boss artifact."""
...
REVIEW_DIRECTORY = REPOSITORY / "docs/art_reviews/bosses_premium_v2"
AUDIT_PATH = REVIEW_DIRECTORY / "bosses_audit.json"
PILOT_REVIEW = "docs/art_reviews/PREMIUM_V2_PILOT_REVIEW.md"
EXPECTED = {
    "ancient_golem": ("heartstone-colossus-v2", "premium-heavy-humanoid-v2", "ancient-golem-ground-slam-v2", "ground_slam"),
    "thorn_matriarch": ("briar-sovereign-v2", "premium-rooted-caster-v2", "thorn-matriarch-thorn-cage-v2", "thorn_cage"),
    "ember_wyrm": ("furnace-wyrm-v2", "premium-winged-wyrm-mapped-v2", "ember-wyrm-flame-sweep-v2", "flame_sweep"),
    "void_knight": ("abyss-champion-v2", "premium-armored-humanoid-v2", "void-knight-void-charge-v2", "void_charge"),
}
EXPECTED_SHEETS = {
    "bosses_lineup.png",
...
def main() -> None:
    ...
    if len(candidate_by_key) != len(candidate_assets):
        raise ValueError("Duplicate boss candidate key")
    if set(candidate_by_key) != set(EXPECTED):
        raise ValueError(
            f"Boss candidate key mismatch: {sorted(set(candidate_by_key) ^ set(EXPECTED))}"
        )
    ...
    if missing:
        raise ValueError(f"Committed catalog is missing Bosses: {sorted(missing)}")
    ...
    catalog["generatedBatch"] = "bosses"
    catalog["assets"] = [by_key[key] for key in sorted(by_key)]
    write_json(catalog_path, catalog)
    print(
        f"Promoted exactly {len(EXPECTED)} reviewed premium-v2 Bosses "
...
def validate_candidate_payload(
    ...
    for key, (revision, rig_profile, animation_profile, signature_attack) in EXPECTED.items():
        asset = candidate_by_key[key]
        expected_fields = {
            "key": key,
            "family": "boss",
            ...
            "unique_attack": signature_attack,
        }
        ...
        if not isinstance(landmarks, list) or len(landmarks) < 4 or not all(landmarks):
            raise ValueError(f"Candidate silhouette-landmark contract mismatch: {key}")
...
def validate_review_evidence(
    ...
    if not AUDIT_PATH.is_file():
        raise FileNotFoundError(f"Missing boss audit: {AUDIT_PATH}")
    actual_sheets = {path.name for path in REVIEW_DIRECTORY.glob("*.png")}
    if actual_sheets != EXPECTED_SHEETS:
        raise ValueError(
            "Boss review-sheet set mismatch: "
            f"missing={sorted(EXPECTED_SHEETS - actual_sheets)}, "
            f"unexpected={sorted(actual_sheets - EXPECTED_SHEETS)}"
        )
    audit = read_json(AUDIT_PATH)
    if audit.get("schemaVersion") != 1 or audit.get("batch") != "bosses-premium-v2":
        raise ValueError("Unexpected boss audit contract")
    if audit.get("expectedKeys") != list(EXPECTED):
        raise ValueError("Boss audit key order mismatch")
    if audit.get("frameContract") != EXPECTED_CLIPS:
        raise ValueError("Boss audit frame contract mismatch")
    if audit.get("minimumUniqueVisibleFrames") != MIN_UNIQUE:
        raise ValueError("Boss audit motion-diversity contract mismatch")
    if audit.get("candidateManifestSha256") != sha256_file(candidate_manifest_path):
        raise ValueError("Boss candidate-manifest provenance mismatch")
    ...
    if set(recorded_sheets) != EXPECTED_SHEETS or audit.get("reviewSheetCount") != len(EXPECTED_SHEETS):
        raise ValueError("Boss review-sheet evidence mismatch")
    for name, record in recorded_sheets.items():
        ...
        if record != {"bytes": path.stat().st_size, "sha256": sha256_file(path)}:
            raise ValueError(f"Boss review-sheet hash mismatch: {name}")

    records = audit.get("assets", [])
    by_key = {record["key"]: record for record in records}
    if len(by_key) != len(records) or set(by_key) != set(EXPECTED):
        raise ValueError("Boss audit asset set mismatch")
    for key, (revision, rig_profile, animation_profile, signature_attack) in EXPECTED.items():
        record = by_key[key]
        ...
        expected_values = {
            ...
            "rigProfile": rig_profile,
            "animationProfile": animation_profile,
            "signatureAttack": signature_attack,
        ...
        for field, expected in expected_values.items():
            if record.get(field) != expected:
                raise ValueError(f"Boss audit provenance mismatch: {key} {field}")
        ...
        for clip, count in EXPECTED_CLIPS.items():
            ...
            if clip_record.get("frameCount") != count:
                raise ValueError(f"Boss audit frame count mismatch: {key} {clip}")
            if int(clip_record.get("uniqueVisibleFrames", 0)) < MIN_UNIQUE[clip]:
                raise ValueError(f"Boss audit motion diversity mismatch: {key} {clip}")
    ...
    exact_summary = {
        ...
        "decodedBytes": 4 * 2048 * 1024 * 4,
        "decodedBudgetBytes": 48 * 1024 * 1024,
    }
    for field, expected in exact_summary.items():
        if summary.get(field) != expected:
            raise ValueError(f"Boss audit summary mismatch: {field}")
    assert_margins(summary.get("minimumAlphaMargins"), "boss batch")
    if not 1_200 <= int(summary.get("minimumTriangles", 0)) <= int(summary.get("maximumTriangles", 0)) <= 6_000:
        raise ValueError("Boss audit triangle summary mismatch")
    if int(summary.get("minimumMeshParts", 0)) < 45:
        raise ValueError("Boss audit mesh-part summary mismatch")
    if int(summary.get("minimumMaterialCount", 0)) < 8:
        raise ValueError("Boss audit material summary mismatch")
    return audit
