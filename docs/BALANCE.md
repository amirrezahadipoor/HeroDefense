...
### Phase 18.4 interim state (waves 101–200 present, not yet tuned)

The run now spans 200 waves. `BalanceSimulator` plays the planting ceremony instantly (`WaveLifecycleSystem.completePlantingCeremony`) and reports `reachedFinalWave` for the full run; `reachedWave100()` remains the tuned-half milestone. Until the final Phase 18.4 rebalance lands, the acceptance gate and the forced-card regression evaluate **waves 1–100 only** (`GATE_WAVE = PLANTING_WAVE`), which keeps the Phase 17 curve locked while the second half is still built. The unrestricted 1–200 baseline currently finishes (4/4 seeds) but with 18.0% average gross damage and single-wave spikes above 35% after wave 180 — this is the explicit input to the final rebalance task, which will re-extend the gate to Wave 200.
